...
docker-compose exec php-apache apt curl wget vim
docker-compose exec php-apache curl -sS https://getcomposer.org/installer | sudo php -- --install-dir=/usr/local/bin --filename=composer
